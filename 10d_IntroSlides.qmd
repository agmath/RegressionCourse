...
**Pre-Modeling Assumptions:** Penguin body mass is associated with each of the four predictors in a linear manner, the predictors are not correlated with one another, and all other possible features are sources of unmodeled variation in body mass.
...
+ Training RMSE (`sigma`) is about 388.41, so we expect our model's typical prediction error to be about388.41 grams.
...
+ Test RMSE is about 376.7, so we expect our model's typical prediction error on new observations to be about 376.7 grams.
