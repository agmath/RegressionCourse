...
Okay, both of the remaining predictors, `flipper_length_mm` and `year` are statistically significant. This gives us our "*final*" model form of $\mathbb{E}\left[\text{body_mass_g}\right] = \beta_0 + \beta_1 \cdot\text{flipper_length_mm} + \beta_2\cdot\text{year}$, where the estimated model has $\beta_0\approx$ `r format(round((coef_df %>% filter(term == "(Intercept)") %>% pull(estimate)), 2))`, $\beta_1\approx$ `r round((coef_df %>% filter(term == "flipper_length_mm") %>% pull(estimate)), 2)`, and $\beta_2\approx$ `r round((coef_df %>% filter(term == "year") %>% pull(estimate)), 2)`.
