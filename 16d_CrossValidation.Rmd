...
+ To split our training data into cross-validation folds, we use `vfold_cv()`
+ To fit our data using cross-validation we use `fit_resamples()` in place of the `fit()` function we have been using up to this point.
+ We obtain cross-validation performance metrics by calling `collect_metrics()` on our "fitted" model.
