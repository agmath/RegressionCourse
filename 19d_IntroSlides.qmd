...
> We see that the test RMSE for our third degree model is `r round(test_error, 2)`, so we expect our model's typical prediction error to be on the order of `r round(test_error, 2)`
...
Depending on the test set, we could be claiming that our model's typical prediction error is on the order of $149.5$ or $246.3$ -- that's a big difference
...
1. Randomly split available data into *training* and *test* sets
...
3. For each individual fold...
    ...
    + Forget the learned coefficients
...
> We see that the *cross-validation* RMSE is about `r round(cv_rmse, 2)`, with a standard error of about `r round(cv_rmse_sd, 2)`, so we can be about 95% confident typical body mass prediction error will be on an order somewhere between `r round(cv_rmse, 2)` $\pm 2\cdot\left(\right.$ `r round(cv_rmse_sd, 2)`$\left.\right)$g, or equivalently, somewhere between `r round(cv_rmse - 2*cv_rmse_sd, 2)`g and `r round(cv_rmse + 2*cv_rmse_sd, 2)`g. 
...
+ In `{tidymodels}`, we 
  ...
  4. once you have a best model (lowest cv-RMSE, highest cv-R-squared, etc.), then `fit()` that model to your training data as we've done in the past.

## Let's Do This!

Let's try out cross-validation on the AirBnB Europe data that we were using earlier this semester or with the data you are using for our competition assignments.
