...
+ **General Goal:** Build a model $\displaystyle{\mathbb{E}\left[y\right] = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \cdots + \beta_k x_k}$ to predict $y$ given features $x_1, \cdots, x_k$.
...
$$\sum_{i = 1}^{n}{\left(y_{\text{obs}} - y_{\text{pred}}\right)^2}$$

## How it works

In this case, we have $\mathbb{E}\left[y\right] = \beta_0 + \beta_1\cdot x$ and we find $\beta_0$ (intercept) and $\beta_1$ (slope) to minimize the quantity

$$\sum_{i = 1}^{n}{\left(y_{\text{obs}} - \left(\beta_0 + \beta_1\cdot x_{\text{obs}}\right)\right)^2}$$
...
## But our predictions are all wrong...literally!
...
+ I'll walk you through every step, via a series of assignments.
+ You'll take data provided by *Door Dash* and build models.
+ Kaggle will assess your models' predictions (and also those predictions of your colleagues).
+ We'll have a live leaderboard, using only part of the competition data so you know approximately where you stand.
...
+ We've all done homework assignments, but we almost never think of the work on the assignment after everything is turned in and graded.
+ The competition assignments and environment ask you to iterate on previous work.
...
+ You'll be motivated to improve your model even if you aren't working on an assignment.
...
+ Six assignments -- each focusing on part(s) of the analytics report
...
**Homework:** Competition Assignment 1 -- join the competition, read the details, download the data, and write a *Statement of Purpose*.