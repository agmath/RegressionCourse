...
## Aside: Training and Testing Data

<div style="font-size:18pt">

:::{.fragment}

At our Day 2 discussion, we mentioned exploratory versus confirmatory analyses and that we'd consistently come at problems from the exploratory setting. 

:::

:::{.fragment}

Because of this, we should get in the habit of splitting our data into *training* and at least one *validation*/*test* set.

:::

:::{.fragment}

Before we move forward, I'll have you split your AirBnB Europe data into a training and test set. Use code like the following:
...
+ Replace `data` with the name of the data frame you want to split.
+ The `initial_split()` function comes from `{tidymodels}` and marks rows as belonging to *training* or *testing* sets.
...
From here on, we'll use `train` for all of our exploratory and model construction work. We'll reserve the `test` data for unbiased model assessment.
...
Use this time to continue playing with the `airbnb` pricing data, but be sure to only explore your `train`ing set.
