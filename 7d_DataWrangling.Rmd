...
items <- items %>%
  append(c("Outdoor Furniture", "tv and media furniture"))

products <- tibble(items = items, prices = c("$125", "$675", "$1,350", "$475", "$725", "$725", "$475"))
...
orders <- tibble(client_id = sample(1:10, size = numRows, replace = TRUE), product = sample(products$items, size = numRows, replace = TRUE), quantity = 1 + rpois(numRows, lambda = 2))
...
Run all of the code in the initial code chunk, line by line. You will likely encounter an error on line 12 because you don't have the `randomNames` package installed on your machine. In line 11, un-comment `install.packages("randomNames")` and run it. Now re-run line 12 and continue running each line in the code chunk. You can examine the code if you like, but it is not necessary. 
...
Take a look at the `products` data frame. The data frame only has seven rows, but among those rows are seemingly duplicated records. Luckily, while the item categories are duplicated (with different capitalization), the prices are consistent. We have several options available:
...
There are certainly other things we could do as well. These three approaches are shown below.

```{r}
#Filter out the duplicated rows
products %>%
  filter(!(items %in% c("Outdoor Furniture", "tv and media furniture")))
...
products %>%
  mutate(items = case_when(
    ...
    items == "tv and media furniture" ~ "TV & media furniture",
    TRUE ~ items
  )) %>%
  distinct()
```

The `case_when()` approach is more applicable than simply removing the rows, because it isn't often that we would actually like to remove rows. That is the case here, though. Choose your favorite method and overwrite the `products` data frame with a version of itself that does not have the duplicated rows.
...
Moving between wide and long formats is a pretty useful skill. I use it often when I am summarizing data and would like to dispay a summary table in a report. These summary tables sometimes look more pleasing and are easier to digest in a wide format.
