...
```{r}
set.seed(300)
penguin_splits <- initial_split(penguins, prop = 3/4)
...
Using this new data frame, we can compare the *truth* (know, actual `body_mass_g`) to our *predictions* (`.pred`) to assess the quality of predictions.
...
Once we have a fitted workflow, there's lots we can do with it. We can see the structure of the fitted model.
