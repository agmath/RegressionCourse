...
set.seed(123)
num_pts <- 20
x <- runif(num_pts, 0, 10)
y1 <- 4*x - 1 + rnorm(num_pts, 0, 8)
y2 <- 4*x - 1 + rnorm(num_pts, 0, 30)

num_new <- 10
x_new <- runif(num_new, 0, 10)
y1_new <- 4*x_new - 1 + rnorm(num_new, 0, 8)
...
lin_reg_rec <- recipe(y ~ x, data = data)
lin_reg_wf <- workflow() %>%
...
We'll run one of these tests for each term in our model.

## Confidence Intervals for Model Coefficients

**Reminder:** An approximate 95% confidence interval is between two *standard errors* below and above our *point estimate*.

$$\left(\text{point estimate}\right) \pm 2\cdot\left(\text{standard error}\right)$$
...
fake_coefs %>%
  ggplot() +
  geom_errorbarh(aes(xmin = estimate - 2*std_err, 
                     xmax = estimate + 2*std_err, 
                     y = term, 
                     color = term),
                 lwd = 1.25) +
...
ggplot() +
  geom_point(aes(x = x_new, y = y1_new)) +
...
The equation to obtain confidence intervals for model predictions is somewhat complex because it accounts for having more certainty near the center of the feature space and less certainty on the extremes. I'll omit it here, since we can use R to calculate these intervals.
