...
1. Navigate to our competition site at Kaggle -- you can find the link in our *Welcome Announcement* on BrightSpace and in a post I made on Slack. Either sign in with Facebook, Google, or create an account using an e-mail address.
...
4. Download the two data sets -- you'll need `initial_data.csv` for this assignment and `competition_data.csv` contains the actual competition data and will be used once we learn how to construct models.
...
6. In that notebook, do the following to load the data.
    ...
    + Use `read_csv()` along with the path to the `initial_data.csv` file on your computer to read that file into your notebook. Don't forget to store your data into a named variable so that you can continue to access it. 
...
9. Submit both your Quarto file and the html document you created to BrightSpace using the Competition Assignment 1 submission folder.