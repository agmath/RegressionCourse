...
3. Write code to split the data coming from `dataFA24.csv` into three sets: `train`, `validation` and `test`. You'll need two separate calls to `initial_split()` for this. Be sure to set a seed, with `set.seed()` before each call.
