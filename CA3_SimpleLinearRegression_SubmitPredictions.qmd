...
4. Build a simple linear regression model using what you've learned about the `{tidymodels}` framework. Be sure to use the single predictor that you think is most important in predicting the response variable for the competition.

5. Assess your model's performance using what you've learned about model assessment so far. You should use the `test` data for this purpose, but please do not open the `validation` data yet -- we will be building more models and don't want any knowledge of the `validation` data to infect our modeling decisions.

7. Return to the `setup` chunk and load the `competition_data.csv` data set into your Notebook. Be sure to store this data into a named variable so that you can access it later.
...
8. Use your model to `augment()` the competition data with predictions. Be sure to store the resulting data frame in a named variable -- you may want to use a new variable name so that you don't overwrite your competition data altogether. Additionally, the only two columns you'll need now are the `id` and the predicted response columns. Remember that you can use `rename()` to rename the `.pred` column to match the name of the *response* column that Kaggle is expecting. The chain of commands below may be helpful.
...
10. Navigate to our competition site on Kaggle and use the *Submission* button to make your first submission for scoring on Kaggle. You'll upload your csv file and then see where you fall on the leaderboard. This *public* leaderboard is scored using only 88% of the listings in our competition data set. The final, *private* leaderboard will be scored on the remaining 12% of those hidden listings -- that is, your position on the leaderboard may change when the competition closes before the last week of the semester. **Note:** Your position on the leaderboard is in no way related to your score on these assignments, but I hope that you are motivated to work your way towards the top of the leaderboard as our semester progresses.
