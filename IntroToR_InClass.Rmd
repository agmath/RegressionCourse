...
rents <- read_csv("https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2022/2022-07-05/rent.csv")
...
```{r}
rents %>%
  select(county, price, beds, baths, sqft) %>%
  summary()
...
```{r}
rents %>%
  summarize(biggest_apt = max(sqft, na.rm = TRUE))
...
Why `arrange()` is better than `sort`
...
We can compute lots of summary statistics at once.
...
rents %>%
  ...
  select(city, price, sqft, price_per_sqft) %>%
